...
class wb_enterprise{
	...
	public static function all(){
		...
		$total = db::get_row_count('wb_enterprise', $where);
		$res = db::query("
			select * from wb_enterprise
			where {$where}
			order by {$orderby}
		");
		$row = array();
	...
	public static function ids($_ARG=array()){
		...
		$res = db::query("select * from wb_enterprise where {$where}{$cfg['where']} order by FIELD(`Id`, $id)");
		$row = array();
		$wb_enterprise_id = '0';
		$wb_enterprise_category_id = '0';
		while ($v=db::result($res)) {
			$wb_enterprise_id .= ','.(int)$v['Id'];
	...
	public static function id($_ARG=array()){
		$id = (int)$_ARG['id'];
		$res = db::query("select * from wb_enterprise where Id=$id limit 0,1");
		$row = array(array());
		while ($v=db::result($res)) {
			$row[0] = self::fields($v);
		}
		self::append($row, array(
			'wb_enterprise_id' => (int)$row[0]['Id'],
	...
	public static function detail($_ARG=array()){
		...
		// $row[0] = db::get('wb_enterprise::id', array('id'=>$id));
		if (!$row[0]) {
			return array();
		}
		self::append($row, array(
	...
	public static function editor($_ARG=array()){
		$id = (int)$_ARG['id'];
		$editor = db::editor('wb_enterprise', $id, 'Detail');
		if ($editor) {
			return array(
				array(
					'Name' => '详情内容',
	...
	public static function prev($_ARG=array()){
		$id = (int)$_ARG['id'];
		$row = db::get_one('wb_enterprise', "Id='{$id}'");
		// $row = \saas::$row;
		// $id = $row['Id'];
		$myorder = $row['MyOrder'];
		$add_time = $row['AddTime'];
		$where = "Language='".c('lang')."'";
		if ($_SESSION['saas_prev_next_wb_enterprise_cid'] && $row['wb_enterprise_category_id']) {
			$where .= " and wb_enterprise_category_id='{$row['wb_enterprise_category_id']}'";
		}
		$prev = array();
        $prev[0] = db::get_one('wb_enterprise', $where." and MyOrder='{$myorder}' and AddTime='{$add_time}' and Id>'{$id}'", '*', 'Id asc');
        $prev[0] || $prev[0] = db::get_one('wb_enterprise', $where." and MyOrder='{$myorder}' and AddTime>'{$add_time}'", '*', 'AddTime asc,Id asc');
        $prev[0] || $prev[0] = db::get_one('wb_enterprise', $where." and MyOrder<'{$myorder}'", '*', 'MyOrder desc,AddTime asc,Id asc');
        if ($prev[0]) {
        	$prev[0] = self::fields($prev[0]);
			self::append($prev, array(
				'wb_enterprise_id' => (int)$prev[0]['Id'],
				'wb_enterprise_category_id' => (int)$prev[0]['wb_enterprise_category_id'],
			));
        }
		return $prev[0];
	}
	...
	public static function next($_ARG=array()){
		...
		$next = array();
        $next[0] = db::get_one('wb_enterprise', $where." and MyOrder='{$myorder}' and AddTime='{$add_time}' and Id<'{$id}'", '*', 'Id desc');
        $next[0] || $next[0] = db::get_one('wb_enterprise', $where." and MyOrder='{$myorder}' and AddTime<'{$add_time}'", '*', 'AddTime desc,Id desc');
        $next[0] || $next[0] = db::get_one('wb_enterprise', $where." and MyOrder>'{$myorder}'", '*', 'MyOrder asc,AddTime desc,Id desc');
        if ($next[0]) {
        	$next[0] = self::fields($next[0]);
			self::append($next, array(
				'wb_enterprise_id' => (int)$next[0]['Id'],
				'wb_enterprise_category_id' => (int)$next[0]['wb_enterprise_category_id'],
			));
        }
		return $next[0];
	}
