<?php
p('feedback.index.export') || str::msg(language('notes.no_permit'));

$where = '1=1';
$row = db::get_whole('wb_feedback', $where, '*', 'MyOrder asc, AddTime desc');
$i = 1;
$data_excel = array(// 设置内容
	...
	'ExcelContents' => array(// 内容
		...
		excel::list_array_index($i, '公司'),
		excel::list_array_index($i, '职业'),
...
foreach ($row['row'] as $k => $v) {
	...
	$data_excel['ExcelContents'][] = excel::list_array_index($i, $v['Company']);
	$data_excel['ExcelContents'][] = excel::list_array_index($i, $v['Job']);
...
// d($data_excel);die;
log::manage('feedback/index', '批量导出留言数据');
excel::export($data_excel, 'feedback.'.date('ymd'));// 开始导出
?>