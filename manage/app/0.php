	...
	<div class="mb_50px" cw="1000">
		...
		<ul class="wcb_app_ul">
			...
			foreach ((array)$menu as $k => $v) {
				if ($k=='0' || $k=='_' || $k=='app_store') {
					continue;
				}
				$ary = language('menu.app.'.$k);
			?>
		...
		<div class="wcb_app_more">
			<?=str_replace(
				...
				array(
					language('{/panel.app/}'),
					'<a hr-ef="'.c('manage.permit.allurl.app.app_store._').'">'.language('{/panel.app_store/}').'</a>'
				),
				language('{/panel.more_app/}')
			);?>
		</div>
	</div>
