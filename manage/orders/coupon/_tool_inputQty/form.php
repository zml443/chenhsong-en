	...
	<div class='_dbs_content'>
		...
		<div class="">
			<label class="ly_input mb_10px">
				<input type="text" name="<?=$name?>" value="<?=$row[$name]?:''?>" onkeyup="_tool_inputQty_cb.writeNumber(this)">
				<div class="bg_pane"><?=language('panel.DistributionQty.unit')?></div>
			</label>
			<label class="flex-btn pointer">
				<i class="ly_checkbox lyicon-select-bold mr_10px"></i>
				<input type="checkbox" class="hide" value="1" <?=$row[$name.'Type']?'checked':''?> fn="_tool_inputQty_cb"/>
				<input type="hidden" name="<?=$name?>Type" value="<?=$row[$name.'Type']?>" />
				<span><?=language('panel.DistributionQty.number')?></span>
			</label>
		</div>
		<!-- 结束 -->
	</div>
