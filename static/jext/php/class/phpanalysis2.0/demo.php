...
if($str != '')
{
    ...
    // $okresult = $pa->GetFinallyKeywords();
    ...
    $pa = '';
    $data = explode(' ',$okresult);
    $word = array();
    echo '<pre>';
    foreach($data as $k=>$v){
        $x=explode('/',$v);
        if(isset($x[1])) $x[1]='.'.$x[1];
        else continue;
        // if(!in_array($x[0],$word) && (strstr($x[1],'.n')||strstr($x[1],'.x')||strstr($x[1],'.vn')||strstr($x[1],'.i')||strstr($x[1],'.a')))
        // if(!in_array($x[0],$word) && (strstr($x[1],'.x')))
            @$word[] = $x[0];
    }
    print_r($word);
    echo '</pre>';
