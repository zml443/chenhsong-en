...
bendi.lis = (da)=>{
	return `
		<label class="wcb_fileitem file cur" fn="paixu">
			<input type="checkbox" name="Id[]" class="hide" data-file="${da.Path}" value="${da.Id}" checked fn="bendi.choice" />
...
bendi.filesDeleteId = (ids)=>{
	...
	for (var i=0; i<returnResult.length;i++) {
		if (ids.indexOf(returnResult[i].id)>=0) {
			// console.log(ids, returnResult[i].id)
