...
(function(){
    ...
    function insertSingle(){
        ...
        if(/^<iframe/.test(url)){
            var conUrl = '';
            if(/src=["'][^"']+/i.test(url)){
                conUrl = url.match(/src=["'][^"']+/i)[0].substr(5);
            }
            ...
            editor.execCommand('insertHtml', "<p><iframe class='ued-video' width='"+width.value+"' height='"+height.value+"' src='" + convert_url(url) + "' frameborder='0' scrolling='no' allowfullscreen='allowfullscreen'></iframe></p>");
            /*editor.execCommand('insertvideo', {
                url: convert_url(url),
                width: width.value,
                height: height.value,
    ...
    function createPreviewVideo(url){
        ...
        if(/^<iframe/.test(url)){
            var conUrl = '';
            if(/src=["'][^"']+/i.test(url)){
                conUrl = url.match(/src=["'][^"']+/i)[0].substr(5); //匹配视频单引号跟双引号
            }
            $G("preview").innerHTML = '<div class="previewMsg"><span>'+lang.urlError+'</span></div>'+
