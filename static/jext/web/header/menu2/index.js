class ly_header_menu {
	constructor(el){
		var opt = $.json(el.attr('ly-header-menu2'), 'simple');
		if (opt.color) {
			// 将颜色字符串转成数组
			...
			opt.color = [el.attr('data-color0')||'#fff',el.attr('data-color1')||'#333',el.attr('data-color2')||'#f1f1f1'];
		}
		this.data = {
			el: el,
			bgColor: opt.color[0],
		...
		if($.language.all.length>1){
			var lang_tab = `
				<div class="ly_header_menu_default_nav_li_btn flex-max2">
					${$.language.all.map(v => {return `<span class="${v==$.language.cur?'cur':''}">${$.lang.language.jianxie[v]}</span>`})}
				</div>
...
$(document).ready(()=>{
	$('[ly-header-menu2]').each(function(){
		var a = new ly_header_menu($(this));
		$(this).o('ly-header-menu2', a);
		a.ohtml.find('.ly_header_menu_default').o('ly-header-menu2', a);
	});
});


// 点击出现侧边菜单
$(document).on('click',"[ly-header-menu2]", function(event){
	var el = $(this);
	if ($('.ly_header_menu_ui').is('.cur')) {
		el.o('ly-header-menu2').hide();
	} else {
		el.o('ly-header-menu2').show();
	}
});

// 关闭侧边栏
$(document).on('click',".ly_header_menu_default", function(){
	$(this).o('ly-header-menu2').hide();
});
...
$(document).on('click',' .ly_header_menu_default_nav_li', function(){
	...
	var parent = $(this).parents('.ly_header_menu_default_nav, .ly_header_menu_default_subnav');
	// var name = $(this).attr('data-name');

	// 将点击目标的父级的所有next兄弟去除cur prev
	parent.nextAll().removeClass('cur prev');
	parent.nextAll().find('.ly_header_menu_default_nav_li').removeClass('cur');
...
$(document).on('click',' .ly_header_menu_default_subnav_back', function(){
	...
	var wrap = $(this).parents('.ly_header_menu_default_wrap')
	var el = wrap.children(`[data-index="${index}"]`);
	var prev = el.prevAll('.cur').eq(0);
	el.removeClass('cur');
	prev.removeClass('prev');
	// 副侧边栏subnav没选中时，移出nav的cur
