$('[jextstyle]').append("[canvas-src^='technology,']{background-color: #000;}");
$.eval('threeGUI', function () {
$.task.push(function () {
	_('[canvas-src^="technology,"]').each(function () {

		var thi = $(this);
		var rand_id = $.rand('canvas-src-technology');
		var rand_id2 = $.rand('canvas-src-technology');

		thi.append('<canvas class="absolute max maxw maxh" id="'+rand_id+'" width="1920" height="572"></canvas>');
		thi.append('<canvas class="absolute max maxw maxh" id="'+rand_id2+'" width="1920" height="572"></canvas>');

		const cacheCanvas = document.getElementById(rand_id);
		const canvas = document.getElementById(rand_id2);
