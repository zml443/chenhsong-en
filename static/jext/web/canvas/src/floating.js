var canvas_floating_colors = function(canvasobj, colorvalue, slicesvalue){
...
$('[jextstyle]').append('[canvas-src^="floating,"] canvas{width:100%;height:100%;}');
$.task.push(function () {
    _('[canvas-src^="floating,"]').each(function(i,e,v,c){
        var e = $(this),
            v = ['#008cd6', '#FFFFFF', '#008cd6', '#444444'],
            c = $.json(e.attr('canvas-src').replace(/^[^,]*,(.*)$/,'$1'),'simple');
        v[0] = c.color || v[0];
        v[2] = c.color || v[2];
        canvas_floating_colors(e, v, c.qty);
    });
});
