$('[jextstyle]').append("[canvas-src^='star2,']{background: #000C1C;background: -moz-linear-gradient(top, #000C1C 0%, #15374F 50%, #000C1C 100%);background: -webkit-linear-gradient(top, #000C1C 0%, #15374F 50%, #000C1C 100%);background: linear-gradient(to bottom, #000 0%, #1e1e1e 50%, #000 100%);filter: progid: DXImageTransform.Microsoft.gradient( startColorstr='#000C1C', endColorstr='#15374F', GradientType=0);}");
$.eval('three', function () {
$.task.push(function () {
	_('[canvas-src^="star2,"]').each(function () {
		...
		function init() {
		  ...
		  thi[0].appendChild(renderer.domElement);
		 ...
		 for (var i = 0; i < 500; i++) {
		  ...
		  var mat = new THREE.MeshPhongMaterial({
		    ...
		    map: THREE.ImageUtils.loadTexture($.path+'web/canvas/src/star2.jpg'),
		    bumpMap: THREE.ImageUtils.loadTexture($.path+'web/canvas/src/star2.jpg'),
		    bumpScale:   0.025,
		    specularMap: THREE.ImageUtils.loadTexture($.path+'web/canvas/src/star2.jpg'),
		    specular: new THREE.Color('grey')
		  });
		  
		  var mat3 = new THREE.ShaderMaterial({
		    uniforms: {},
		    vertexShader: 'varying vec3 vNormal; void main() { vNormal = normalize( normalMatrix * normal ); gl_Position = projectionMatrix * modelViewMatrix * vec4( position, 1.0 ); }',//document.getElementById('vertexShader').textContent,
		    fragmentShader: 'varying vec3 vNormal; void main() { float intensity = pow( 0.7 - dot( vNormal, vec3( 0.0, 0.0, 0.5 ) ), 4.0 ); gl_FragColor = vec4( 1.3, 1.0, 1.0, 1.0 ) * intensity; }',//document.getElementById('fragmentShader').textContent,
		    side: THREE.BackSide,
