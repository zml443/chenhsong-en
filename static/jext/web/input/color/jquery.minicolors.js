...
$.task.push(function () {
    _('[color-selector]').each( function() {
      var el = $(this);
      var fn = $.callbackfn(el.attr('fn'), 'move,confirm');
      var val = el.val();
      var log = '';
  $(this).spectrum({
      allowEmpty:true,
      ...
      hide: function (color) {
        ...
        if (log.rgb!=c.rgb) $.eval(fn.confirm, el, c);
      },
  ...
  el.next('.sp-replacer').addClass('ly_input_div').attr({size:el.attr('size')})
  });
});
