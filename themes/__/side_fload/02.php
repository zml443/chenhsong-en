...
<link rel="stylesheet" href="/themes/__/side_fload/css/02.css">

<section class="lysidefloadbox" id="side_fload_02" data-pos="<?=g('wb_service.position')?:'6';?>">
    <?php
    foreach($row as $k => $v){
        if($v['isHide']){
            continue;
        ...
        switch ($v['Type']) {
            ...
            case 'dy':
                echo '
                    <a class="codeBox relative trans flex-max2" href="'.$v['Href'].'" target="'.$v['Target'].'">
            ...
            default:
                echo '
                    <a class="messageBox trans flex-max2" href="'.$v['Href'].'" target="'.$v['Target'].'" >
                        <img class="svg" src="'.$v['Icon'].'" onload="SVGInject(this)">
                    </a>
                ';
        }
...
<script>
     $(document).on('click', '#side_fload_02 .backTop',function(){
        $('html,body').animate({scrollTop:0},500);
    });

    $(document).on('mouseenter', '#side_fload_02 .telBox',function(){
    ...
    $(document).on('mouseleave', '#side_fload_02 .telBox',function(){
        $(this).find('.box').css({'width':55});
    });


    // $(document).on('click', '#side_fload_02 .window_open_consult',function(){
